...
//------------------------------------------------------------------------------------------------
//根据后台选择的值确定select的选定值
function Select2Resolver(){
    this.tagName="select";
}
Select2Resolver.prototype.setWidgetValue = function(widget,fieldValue){
            	if( !widget.getAttribute("multiple") ){
            		for(var index=0; index<widget.length; index++) {	  
			    	    if (widget.options[index].value == fieldValue){
			    	    	widget.options[index].focus();
			    	    }
			        }
            	}
}
Select2Resolver.prototype.setObjectValue = function (widget,data,fieldName){
	            if( !widget.getAttribute("multiple") ){
	            	data[fieldName] = widget.options[widget.selectedIndex].value;
	            }
}
Select2Resolver.prototype.isEmpty = function (widget){
             	return widget.length ==0;
}
